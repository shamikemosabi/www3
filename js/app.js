...
{var date=new Date(value.date);value.newdate=date.toLocaleString();value.htmlMsg=$scope.renderHtmlChat(value.user,value.msg);$scope.displayChat.push(value);}}};$scope.renderHtmlChat=function(user,html_code){var str=getDutyColor(user)+" : "+html_code;return $sce.trustAsHtml(str);};function getDutyColor(user)
...
{return false;}
var d=new Date();var u=getUserName();var string="<font size=1>"+$scope.title+"</font>"+" </br> "+$scope.txtcomment;$scope.chat.$add({msg:string,date:d.getTime(),browser:navigator.userAgent,user:u});$scope.save();$scope.CD=string;};$scope.save=function(){if($scope.txtcomment==null||$scope.txtcomment=="")
{return false;}
